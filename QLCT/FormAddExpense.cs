    ...
    public partial class FormAddExpense : Form
    {
        ...
        public void DrawCommands()
        {
            ...
            btnAdd.Click += (sender, args) =>
            {
                ...
                if (Int64.Parse(cost.Text.ToString()) > Form1.balance)
                {
                    MessageBox.Show("Không đủ tiền! Vui lòng đi kiếm thêm tiền.");
                    return;
                }
                ...
                expense.TenKhoanChi = name.Text.ToString();
                expense.LoaiKhoanChi = type.SelectedItem.ToString();
                expense.ThoiGian = DateTime.Parse(date.Text.ToString());
                expense.SoTien = Int64.Parse(cost.Text.ToString());
                Form1.balance -= expense.SoTien;

                QLCTDataContext db = new QLCTDataContext();
                db.KhoanChis.InsertOnSubmit(expense);
                db.SubmitChanges();
